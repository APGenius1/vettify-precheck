...
def send_email_notification(application_data):
    """Send email in background - doesn't block the user"""
    try:
        msg = MIMEMultipart()
        ...
        Profile Text: {application_data.get('profile_text', 'Not provided')[:200]}
...
def analyze_authority_signal(text):
    ...
    reasons = []
    fixes = []
    decision_words = ['decide', 'lead', 'direct', 'oversee', 'responsible', 'accountable']
    ...
    return {'score': max(0, min(100, score)), 'level': 'Weak Consistency' if score < 70 else 'Moderate', 'reasons': reasons[:2], 'fixes': fixes[:2]}
...
def analyze_narrative_alignment(text):
    ...
    return {'score': max(0, min(100, score)), 'level': 'Fragmented Signal' if score < 75 else 'Aligned', 'reasons': reasons[:2], 'fixes': fixes[:2]}
...
def analyze_visibility_footprint(text):
    ...
    return {'score': max(0, min(100, score)), 'level': 'Under-Optimised' if score < 70 else 'Building', 'reasons': reasons[:2], 'fixes': fixes[:2]}
...
def analyze_validation_signal(text):
    ...
    return {'score': max(0, min(100, score)), 'level': 'Under-Optimised' if score < 70 else 'Moderate', 'reasons': reasons[:2], 'fixes': fixes[:2]}
...
def generate_perception_gap_report(client_data, profile_text):
    ...
    scores = {'Authority Positioning': authority['score'], 'Narrative Alignment': narrative['score'], 'Visibility Footprint': visibility['score'], 'Third-Party Validation': validation['score']}
    ...
    benchmark_data = [["Your Score", f"{int(overall_score)}/100", ""], ["Industry Average (Founders)", "72/100", f"{'+' if overall_score > 72 else ''}{int(overall_score - 72)} vs avg"], ["Top 10% Tier", "88/100", f"Gap: {88 - int(overall_score)} points"]]
    bench_table = Table(benchmark_data, colWidths=[2.0*inch, 1.2*inch, 2.5*inch])
    bench_table.setStyle(TableStyle([('FONTNAME', (0,0), (-1,-1), 'Helvetica'), ('FONTSIZE', (0,0), (-1,-1), 9), ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#0a1628')), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#e2e8f0')), ('TOPPADDING', (0,0), (-1,-1), 8)]))
    story.append(bench_table)
    story.append(Spacer(1, 0.15*inch))
    
    story.append(Paragraph("3. SCORE BREAKDOWN & WHY", section_header))
    for signal, data in [('Authority Positioning', authority), ('Narrative Alignment', narrative), ('Visibility Footprint', visibility), ('Third-Party Validation', validation)]:
    ...
    for i, (action, category) in enumerate(actions[:6]):
        action_data.append([action[:50] + ("..." if len(action) > 50 else ""), category, f"+{points[i] if i < len(points) else 5}", "30-60 days"])
    action_table = Table(action_data, colWidths=[2.5*inch, 1.0*inch, 0.8*inch, 0.8*inch])
    action_table.setStyle(TableStyle([('FONTNAME', (0,0), (-1,-1), 'Helvetica'), ('FONTSIZE', (0,0), (-1,-1), 8), ('BACKGROUND', (0,0), (-1,0), colors.HexColor('#0a1628')), ('TEXTCOLOR', (0,0), (-1,0), colors.white), ('GRID', (0,0), (-1,-1), 0.5, colors.HexColor('#e2e8f0')), ('TOPPADDING', (0,0), (-1,-1), 6)]))
    ...
    story.append(Paragraph(f"Confidential · Prepared for {client_name} · Valid for 60 days", ParagraphStyle('Footer', parent=styles['Normal'], fontSize=7, textColor=colors.HexColor('#8a9bb0'), alignment=1)))
...
def submit_application():
    ...
    # Quick database write (instant)
    conn = get_db()
    app_id = str(uuid.uuid4())[:8]
    conn.execute('''INSERT INTO applications (id, full_name, email, linkedin_url, position, funding_amount, visibility_goal, submitted_at) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
        (app_id, data['full_name'], data['email'], data['linkedin_url'], 
         data['position'], data['funding_amount'], data.get('profile_text', ''), datetime.now().isoformat()))
    conn.commit()
    conn.close()
    
    # Send email in background (doesn't block)
    threading.Thread(target=send_email_notification, args=(data,)).start()
