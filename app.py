...
def generate_intelligence_report(client_data):
    ...
    # PAGE 1 - Header + Executive Summary + Perception Index
    ...
    # PAGE 2 - Credibility Signal Intelligence
    ...
    story.append(Paragraph("Your public-facing identity does not consistently signal decision-making power. In environments where first impressions determine access, weak authority signals create filtering friction.", body_text))
    story.append(Spacer(1, 0.05*inch))
    story.append(Paragraph("Current signals: Competence-focused, contribution-oriented, collaborative tone.", body_text))
    story.append(Paragraph("Missing signals: Decisiveness, domain ownership, directional influence, gatekeeper positioning.", body_text))
    story.append(Spacer(1, 0.05*inch))
    story.append(Paragraph("<b>Interpretation risk:</b> You are likely being categorized as 'operator' rather than 'owner,' 'contributor' rather than 'authority.' This reduces perceived strategic weight in high-stakes contexts.", risk_text))
    story.append(Spacer(1, 0.1*inch))
    ...
    story.append(Spacer(1, 0.05*inch))
    story.append(Paragraph("<b>Effect:</b> Each interaction requires reinterpretation. At elite visibility levels, interpretation friction = opportunity loss = slower trust velocity.", risk_text))
    story.append(Spacer(1, 0.1*inch))
    ...
    story.append(Spacer(1, 0.05*inch))
    story.append(Paragraph("Current validation gaps include:", body_text))
    ...
    story.append(Spacer(1, 0.05*inch))
    story.append(Paragraph("<b>Consequence:</b> Verification friction in high-trust environments. People who need to quickly assess 'who you are' take longer to reach confidence — or move to someone easier to verify.", risk_text))
    story.append(Spacer(1, 0.15*inch))
    
    # PAGE 3 - Strategic Exposure Risks
    ...
    # PAGE 4 - Executive Action Framework + Final Diagnostic
    ...
    story.append(Paragraph("Timeline: 30 days. Impact: Immediate shift in first-impression categorization.", small_text))
    story.append(Spacer(1, 0.08*inch))
    ...
    story.append(Paragraph("Timeline: 60 days. Impact: Reduced friction in sequential interactions.", small_text))
    story.append(Spacer(1, 0.08*inch))
    ...
    story.append(Paragraph("Timeline: 90-120 days. Impact: Increased verification speed in high-trust environments.", small_text))
    story.append(Spacer(1, 0.08*inch))
    ...
    story.append(Paragraph("Timeline: 90 days. Impact: External validation anchors for your authority claims.", small_text))
    story.append(Spacer(1, 0.15*inch))
    ...
    story.append(Spacer(1, 0.05*inch))
    story.append(Paragraph("Correcting this perception gap compounds into:", body_text))
...
<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Vettify | Private Perception Intelligence</title>
